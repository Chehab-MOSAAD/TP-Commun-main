...
class FormulaireRestaurantTest {
	...
	void testGetNumService() {
		assertEquals(NUM_SERVICE, formulaire.getNumeroService(), "Le numero de service devrait etre " + NUM_SERVICE);
	}

	@Test
	void getJour() {
		assertEquals(JOUR, formulaire.getDateReservation() , "Le jour devrait etre " + JOUR);
	}

	@Test
	void getMois() {
		assertEquals(MOIS, formulaire.getMoisReservation(), "Le mois devrait etre " + MOIS);
	}

	@Test
	void setIdentificationEntite() {
		FormulaireRestaurant formulaire2 = new FormulaireRestaurant(JOUR, MOIS, NB_PERSONNES, NUM_SERVICE);
		assertEquals(1, formulaire2.getIdentificationEntite(),
